...
* All features must include appropriate documentation additions or updates.
...
* At the end of every Milestone and Feature you must (in order):
  ...
  2. Ensure that the Docker image builds successfully.
...
* Every feature must end with an "Acceptance Criteria" Milestone. This Milestone must include tasks to:
  1. Ensure that all appropriate documentation (`README.md` and `CLAUDE.md`) is updated as needed for the work done as part of the feature. Documentation should be easily readable, concise, and a match to the style, tone, and verbosity of the existing documentation.
  2. As the last step of every feature, move the feature markdown file from `docs/features/` to `docs/features/completed/`.
